...
*
*	v7.3.2 - Only show 3day forecast size on tile when over 1000
...
*  - Fixed HTML tile formatting when ICAO airport code is specified
*
...
def wu3dayfcst() {
    ...
    my3day << '<table>'
    // Add location info row if ICAO or custom GPS is specified
    if (sforecastLocation) {
        my3day << '<tr><td>' << sl << '</td>'
        my3day << '<td colspan="6" style="text-align:center; font-size:0.9em;">' << sforecastLocation << '</td></tr>'
        my3day << '<tr><td></td><td style="min-width:5%"></td>'
        my3day << '<td>' << "${device.currentValue("forecastTimeName")}" << (todayAlert ? todayAlert : '')
        my3day << '<td style="min-width:5%">'
        my3day << '<td>' << "${device.currentValue('tomorrow')}" << (tomorrowAlert ? tomorrowAlert : '')
        my3day << '<td style="min-width:5%">'
        my3day << '<td>' << "${device.currentValue('dayAfterTomorrow')}"
    } else {
        my3day << '<tr>' 
        my3day << '<td>' << sl
        my3day << '<td style="min-width:5%">'
    ...
    // Build rain history section separately so we can remove it if needed
    StringBuilder rainHistorySection = new StringBuilder()
    ...
    if (rainhistory) 
    {
        ...
        switch(raindaysdisplay) {        
            case "No selection": 
                rainHistorySection << '<tr style="font-size:75%"> <td colspan="7">' << iconSunrise << strSunrise << ' ' << iconSunset << strSunset << ' @ ' << lastObsTime
                break; 
            case "None": 
                rainHistorySection << '<tr style="font-size:75%"> <td colspan="7">' << iconSunrise << strSunrise << ' ' << iconSunset << strSunset << ' @ ' << lastObsTime
                break; 
             case "Yesterday": 
                rainHistorySection << '<tr style="font-size:75%"> <td colspan="7">' << 'Yesterdays Rain:' << " ${device.currentValue('precip_Yesterday')} " << iconSunrise << strSunrise << ' ' << iconSunset << strSunset << ' @ ' << lastObsTime
                break; 
             case "Last 2/3-Days": 
                rainHistorySection << '<tr style="font-size:75%"> <td colspan="7">' << s1stHeader << " ${device.currentValue('precip_Last3Days')} " << iconSunrise << strSunrise << ' ' << iconSunset << strSunset << ' @ ' << lastObsTime
                break; 
             case "Last 4/5-Days": 
                rainHistorySection << '<tr style="font-size:75%"> <td colspan="7">' << s2ndHeader << " ${device.currentValue('precip_Last5Days')} " << iconSunrise << strSunrise << ' ' << iconSunset << strSunset << ' @ ' << lastObsTime
                break; 
             case "Last 6/7-Days": 
                rainHistorySection << '<tr style="font-size:75%"> <td colspan="7">' << s3rdHeader << " ${device.currentValue('precip_Last7Days')} " << iconSunrise << strSunrise << ' ' << iconSunset << strSunset << ' @ ' << lastObsTime
                break; 
             default: 
                rainHistorySection << '<tr style="font-size:75%"> <td colspan="7">' << iconSunrise << strSunrise << ' ' << iconSunset << strSunset << ' @ ' << lastObsTime
                break; 
    ...
    // Add rain history section
    my3day << rainHistorySection.toString()
    ...
    java.lang.Integer lenmy3day = finalHtml.length()
    
    // SMART TRUNCATION: If over limit, rebuild without rain sections
    if (lenmy3day > MAX_TILE_LENGTH) {
        if(txtEnable){log.warn "3-day tile too long (${lenmy3day} chars), rebuilding without rain data"}
        
        // Rebuild without rain history in the bottom row
        my3day = new StringBuilder()
        my3day << '<table>'
        ...
        // Skip "Chance Precip" row entirely in truncated version
        
        my3day << '<tr><td colspan="7">'  //blank line
        
        // Minimal bottom row - just sunrise/sunset/time, NO rain history
        ...
        my3day << '</table>'
        finalHtml = my3day.toString()
        lenmy3day = finalHtml.length()
        
        if(txtEnable){log.info "Rebuilt tile length: ${lenmy3day} chars"}
    }
    ...
    if (lenmy3day > 1000 && lenmy3day < MAX_TILE_LENGTH) {
        finalHtml = finalHtml.substring(0, finalHtml.length() - 8) + ' {' + lenmy3day + '}</table>'
    }
